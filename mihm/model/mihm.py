...
class MLPEnsemble(nn.Module):
    def __init__(self, 
        ...
        self.vae = vae
        self.output_mu_var = output_mu_var
        self.models = nn.ModuleList([MLP(layer_input_sizes, vae=vae, dropout=dropout, device=device, ensemble=True) for _ in range(n_models)])
...
class IndexPredictionModel(nn.Module):
    def __init__(
        ...
        svd: bool = False,
        svd_matrix: Union[None, torch.Tensor, Sequence[torch.Tensor], np.ndarray, Sequence[np.ndarray]]=None,
        ...
        if self.batch_norm:
            self.bn = nn.BatchNorm1d(self.num_models, affine=False)
        ...
        if self.num_models == 1:
            ...
            if n_ensemble > 1:
                self.mlp = MLPEnsemble(n_ensemble, layer_input_sizes, vae=self.vae, output_mu_var=output_mu_var, dropout=dropout, device=device)
            else:
                self.mlp = MLP(layer_input_sizes, vae=self.vae, output_mu_var=output_mu_var, dropout=dropout, device=device)
            
        else:
            self.num_layers = [len(hidden_layer_sizes[i]) for i in range(self.num_models)]
            # final layer gets added later if add_final_layer = True
            self.mlp = []
            for i in range(self.num_models):
                layer_input_sizes = [num_moderators[i]] + hidden_layer_sizes[i]
                if n_ensemble > 1:
                    self.mlp.append(MLPEnsemble(n_ensemble, layer_input_sizes, vae=self.vae, output_mu_var=output_mu_var, dropout=dropout, device=device))
                else:
                    self.mlp.append(MLP(layer_input_sizes, vae=self.vae, output_mu_var=output_mu_var, dropout=dropout, device=device))

    def forward(self, moderators: Union[torch.Tensor, Sequence[torch.Tensor]]):
        if self.num_models == 1:
            if self.svd:
                moderators = torch.matmul(
                    moderators, self.svd_matrix[:, : self.k_dim]
                )
            if self.vae:
                if not self.training:
            ...
            if self.svd:
                moderators = [torch.matmul(
                    moderators[i], self.svd_matrix[i][:, : self.k_dim[i]]
                ) for i in range(self.num_models)]
...
class MIHM(nn.Module):
    def __init__(
        ...
        vae:bool=True,
        output_mu_var: bool = True,
        interation_direction: str = "positive",
        n_ensemble:int = 1
    ):
        ...
        self.dropout_rate = dropout
        self.index_prediction_model = IndexPredictionModel(num_moderators, 
                                                           hidden_layer_sizes, 
                                                           svd=svd, 
        ...
        self.mmr_parameters = [self.focal_predictor_main_weight, self.predicted_index_weight]
        self.mmr_parameters.extend([param for param in self.controlled_var_weights.parameters()])
        ...
        self.interaction_direction = interation_direction
    ...
    def forward(self, moderators, focal_predictor, controlled_vars):
        ...
        if self.vae:
            if not self.training:
                ...
                if self.output_mu_var:
                    predicted_index, mu, log_var = self.index_prediction_model(moderators)
                else:
        ...
        if self.interaction_direction == "positive":
            predicted_interaction_term = (
                torch.abs(self.predicted_index_weight) * focal_predictor * predicted_index
            )
            ...
            raise ValueError("interaction direction must be either positive or negative!!!")
        ...
        if self.vae:
            if not self.training:
                ...
                if self.output_mu_var:
                    return predicted_epi, mu, log_var
                else:
                    return predicted_epi
        else:
            return predicted_epi