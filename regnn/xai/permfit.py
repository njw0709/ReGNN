...
def permfit_feature_importance_regnn(
    model: ReGNN,
    ...
    if metric == "mse":
        base_error = torch.mean((pred - sample["outcome"]) ** 2).item()  # baseline MSE
    elif metric == "mae":
        base_error = torch.mean(
            torch.abs(pred - sample["outcome"])
        ).item()  # baseline MAE
    else:
        raise ValueError("Unsupported metric. Use 'mse' or 'mae'.")

    X_tensor = sample["moderators"]
    n_samples, _, n_features = X_tensor.shape
    importance_table = []
    ...
    for j in tqdm(range(n_features)):
        ...
        for _ in range(n_permutations):
            ...
            if metric == "mse":
                err = torch.mean((perm_pred - sample["outcome"]) ** 2).item()
            else:  # 'mae'
                err = torch.mean(torch.abs(perm_pred - sample["outcome"])).item()
        ...
        error_diffs = perm_errors - base_error
